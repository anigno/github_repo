...
namespace AfaDataExtraction.Contango
{
    public class ContangoExtractor
    {
        ...
        private const string REGEX_VAL1 = "ticker=VIX/Z4([^>]*>){4}(?<DATA>[0-9.,]*)";
        private const string REGEX_VAL2 = "ticker=VIX/F5([^>]*>){4}(?<DATA>[0-9.,]*)";
        private const string REGEX_VAL3 = "ticker=VIX/G5([^>]*>){4}(?<DATA>[0-9.,]*)";
        private const string URL1 = "http://www.cboe.com/micro/VIX/vixintro.aspx";
		#endregion (------  Constants  ------)

		#region (------  Fields  ------)
        protected readonly WebClient m_client = new WebClient();
        ...
        private readonly Regex m_regex3 = new Regex(REGEX_VAL3, RegexOptions.Singleline | RegexOptions.IgnoreCase);
        private static readonly ILog s_log = LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        protected readonly UTF8Encoding m_utf8Encoder = new UTF8Encoding();
		#endregion (------  Fields  ------)
        ...
        public ContangoExtractedData ExtractLastContangoData()
        {
            try
            {
                ...
                Match match3 = m_regex3.Match(pageData);
        ...
        private string getPageData(string p_url)
        {
            try
            {
                byte[] bytes = m_client.DownloadData(p_url);
                return m_utf8Encoder.GetString(bytes);
            }
            catch (Exception ex)
            {
