...
{
    public class AfaSystemCIntraDayLong<T> : AfaSystemFirstBase<T> where T : CIntraDayLongConfiguration, new()
    {
        #region (------  Fields  ------)
        ...
        #region (------  Constructors  ------)
        public AfaSystemCIntraDayLong(string p_username, int p_key, string p_filename, ContangoManager p_contangoManager)
        ...
        protected override FirstCalculatorResult[] StartProcessingSpecific(string p_symbolName, AfaDataExtraction.SymbolExtractedData[] p_symbolExtractedDataArray)
        {
            ...
            for (int index = 0; index < results.Length - 1; index++)
            {
                ...
                float startValue = AfaSystemContangoHelper.SetSignalContangoLongs(current, previous, config);
                ...
                if (current.SignalType != SignalTypeEnum.None)
                {
                    ...
                    //Check miss
                    current.ProfitPer =  current.FirstAnalyzeResultItem.SymbolExtractedDataItem.Low / startValue-1;
                    ...
                    {
                        //Check hit
                        current.ProfitPer = current.FirstAnalyzeResultItem.SymbolExtractedDataItem.Close/startValue-1;
