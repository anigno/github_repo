...
Public Sub updateData()
    ...
    'check date
    willUpdate = True
    iDate = X.Worksheets(3).Cells(a - 1, 25)
    If iDate = Date Then
        a = a - 1
    ...
    If willUpdate = True Then
        X.Worksheets(3).Cells(a, 25) = Date
        X.Worksheets(3).Cells(a, 26) = Time
    End If
    ...
    End If
    X.Worksheets(3).Range("E" & Trim(Str(a - 1)) & ":X" & Trim(Str(a - 1))).Copy _
        Destination:=Worksheets(3).Range("E" & Trim(Str(a)) & ":X" & Trim(Str(a)))
    'get system value
