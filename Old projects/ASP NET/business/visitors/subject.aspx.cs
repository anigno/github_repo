...
namespace business.visitors
{
	public class subject : System.Web.UI.Page
	{
		...
		{
			subjectKey=Request.QueryString.Get("subjectKey").ToString();
			conn.createSelect("tblSubjects","*","fldKey="+subjectKey,"","");
			DataTable table=conn.runSelect();
			val+=table.Rows[0]["fldName"].ToString()+"<br><br>";
			val+=table.Rows[0]["fldText"].ToString();
			...
			double widthFactor=Global.MAX_BIG_IMAGE_WIDTH/(double)image.Width;
