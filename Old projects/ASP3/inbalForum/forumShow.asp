...
<%if iUsername="administrator" or iUsername="inbal" then%>		
<a title="<%=RS("txtMessage")%>" href="editMessage.asp?key=<%=rs("key")%>&amp;forumId=<%=forumId%>&amp;iRoot=<%=iRoot%>">
<%else%>
<a title="<%=RS("txtMessage")%>" href="showMessage.asp?key=<%=rs("key")%>&amp;forumId=<%=forumId%>&amp;iRoot=<%=iRoot%>">
<%end if%>
