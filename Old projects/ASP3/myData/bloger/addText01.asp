...
'get fields data
subject=myRequest("txtSubject")
header=myRequest("txtHeader")
text=myRequest("txtText")
showAll=myRequest("showAll")
if showAll="" then showAll="off"
key=Request.QueryString("key")
'update tblText
	conn=openDB("bloger.mdb")
	set rs=getRS()
	if key="-1" then
		sql="insert into tblText (iUsername,iSubjectNumber,iHeader,iText,iShowToAll,iPicture) values ("
		...
		sql=sql & "iUsername='" & Session("username") & "',"
		sql=sql & "iSubjectNumber=" & subject & ","
		sql=sql & "iHeader='" & header & "',"
		sql=sql & "iText='" & fixText(text) & "',"
		sql=sql & "iShowToAll=" & showAll & ","
		sql=sql & "iPicture='" & fName & "'"
		sql=sql & " where iKey=" & key
	end if
	rs.open sql,conn 
	Response.Redirect "data.asp?publicKey=0"	
