        ...
        private SymbolExtractedData[] extractDataSpecific(string p_symbolWebName, DateTime p_dateFrom, DateTime p_dateTo)
        {
            ...
            try
            {
                ...
                if (p_dateFrom <= p_dateTo)
                {
                    ...
                    if (s_knownSymbolsHistory.ContainsKey(upperTrimmedSymbolName+".HIS"))
                    {
                        knownHistoryData = s_knownSymbolsHistory[upperTrimmedSymbolName + ".HIS"].Where(p_extractedData => { return p_extractedData.DateRead >= p_dateFrom && p_extractedData.DateRead <= p_dateTo; }).ToArray();
